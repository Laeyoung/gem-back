...
function generateTypesFile(models: ProcessedModel[]): string {
  ...
  const unionType = models
    .map((m) => {
      const comment =
        m.classification !== 'stable' ? `  /** ⚠️ ${m.classification.toUpperCase()} - ${m.displayName} */\n` : '';
...
async function main() {
  try {
    console.log('🔧 Generating TypeScript code from model data...\n');
    ...
    console.log(`${typesChanged ? '✨' : '✓'} Generated ${TYPES_OUTPUT} ${typesChanged ? '(changed)' : '(unchanged)'}`);

    fs.writeFileSync(CONFIG_OUTPUT, configContent);
    console.log(`${configChanged ? '✨' : '✓'} Generated ${CONFIG_OUTPUT} ${configChanged ? '(changed)' : '(unchanged)'}`);

    if (typesChanged || configChanged) {
      console.log('\n📝 Model list updated:');
      processedModels.forEach((m) => {
        const badge =
          m.classification === 'stable' ? '✅' : m.classification === 'preview' ? '⚠️' : '🧪';
      ...
      console.log('\n✅ No changes detected. Model list is up to date.');
    }
